/**
 * Face Tracking Kids — Etsy Package Builder
 ...
 *
 * Produces:
 *   etsy-package/FaceTrackingKids-v1.0.0/          (folder)
 *   etsy-package/FaceTrackingKids-v1.0.0.zip       (final ZIP)
...
const ZIP_DIR = resolve(PKG, `FaceTrackingKids-${VERSION}`);
...
async function main() {
  console.log(`\n🎮 Face Tracking Kids Etsy Package Builder — ${VERSION}\n`);
  ...
  const zipPath = join(PKG, `FaceTrackingKids-${VERSION}.zip`);
  if (existsSync(zipPath)) rmSync(zipPath);
  let zipped = false;
  try { execSync(`cd "${PKG}" && zip -r "${zipPath}" "FaceTrackingKids-${VERSION}/"`, { stdio: 'pipe' }); zipped = true; } catch {}
  if (!zipped) try { execSync(`cd "${PKG}" && ditto -c -k --keepParent "FaceTrackingKids-${VERSION}" "${zipPath}"`, { stdio: 'pipe' }); zipped = true; } catch {}
