 ...
 * micro:bit sensor sessions (Smile, wink, or tilt, tilt, head pitch, etc.).
 *
 ...
 *
 * Each CSV matches the format produced by the app's Signals panel CSV export:
 ...
 *   - head pitch-walk-north     (heading ~10° drifting)
...
const SCENARIOS = {
  ...
  'head pitch-walk-north': ({ duration_s = 60, hz = 5 }) => {
    const out = [];
    let h = 8 + gauss(0, 3);
    for (let i = 0; i < duration_s * hz; i++) {
      const t = i * (1000 / hz);
      ...
      if (h < 0) h += 360;
      if (h >= 360) h -= 360;
      out.push([t, 'head pitch', +h.toFixed(1)]);
    }
    return out;
  },
...
const plan = [
  ...
  { scenario: 'head pitch-walk-north', n: 9 },
...
for (const { scenario, n } of plan) {
  for (let i = 1; i <= n; i++) {
    ...
    if (scenario === '5-second-bursts') {
      extras.sensor = ['temp', 'light', 'sound', 'head pitch'][i % 4];
    }
...
\`Graph\` tab of ${'Face Tracking'} via its "Load CSV" button.
...
| \`head pitch-walk-north\` | ${plan.find(p => p.scenario === 'head pitch-walk-north').n} | Circular data, drift analysis |
