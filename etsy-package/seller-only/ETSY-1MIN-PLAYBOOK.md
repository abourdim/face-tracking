...
## Script Template A — "Face Tracking — MediaPipe Face Mesh · 11 Live Signals · 10 Detections · 12 Voice Locales · Chrome / Edge" (hero demo)
...
|---|---|---|
| 0–3 s | *"Face Tracking — MediaPipe Face Mesh · 11 Live Signals · 10 Detections · 12 Voice Locales · Chrome / Edge — in the browser, in 60 seconds."* | Close-up of the real board with the X icon on the LEDs. Your hand holds it. |
...
| 20–30 s | *"Record a full lesson. Export CSV. Drop into a lab report."* | Signals panel → Record → Stop → Save Session → CSV. |
...
| 3–10 s | *"The micro:bit drives the landmark. The 3D model mirrors the real robot."* | Tab 6 (face landmark). Real robot next to the laptop. Robot in 3D follows. |
| 10–20 s | *"D-pad in the browser → arrows on the LED → commands to the motors."* | Tab 4 (Detections). Press ⬆ → LED arrow. Robot crawls forward. |
...
- Use the listing **title** to echo the video's first words. If you say *"Face Tracking — MediaPipe Face Mesh · 11 Live Signals · 10 Detections · 12 Voice Locales · Chrome / Edge"*, the listing title starts with the same opening words.
...
```
Browser-based computer vision tool.  — all from Chrome. No install, no account.
```
